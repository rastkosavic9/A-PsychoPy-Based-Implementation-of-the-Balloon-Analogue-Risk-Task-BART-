...
import random
import math
from psychopy import core, data, event, gui, sound, visual, monitors


# Global experiment clock for event timing
experiment_clock = core.Clock()

# ---------- Paths ----------
# Ensure outputs go under ./data; you will keep this folder empty in GitHub
os.makedirs('data', exist_ok=True)

TRIALS_CSV   = os.path.join('data', 'trials.csv')
SUBJECTS_CSV = os.path.join('data', 'subjects.csv')
BLOCKS_CSV   = os.path.join('data', 'blocks.csv')
EVENTS_CSV = os.path.join('data', 'events.csv')

# Assets in split subfolders
ASSETS_DIR        = 'assets'
SOUNDS_DIR        = os.path.join(ASSETS_DIR, 'sounds')
POP_IMG_DIR       = os.path.join(ASSETS_DIR, 'pop')

COLLECT_SOUND_FILE = os.path.join(SOUNDS_DIR, 'collect_sound.ogg')
POP_SOUND_FILE     = os.path.join(SOUNDS_DIR, 'pop.ogg')
POP1_PNG           = os.path.join(POP_IMG_DIR, 'pop1.png')
POP2_PNG           = os.path.join(POP_IMG_DIR, 'pop2.png')

# ---------- Appearance ----------
BG_COLOR     = (-1.0000, -1.0000, -1.0000)  # window background
CARD_FILL    = [0.20, 0.20, 0.20]  # dark-grey cards/panels
CARD_OPACITY = 0.96

# Text color policy
COLOR_ON_CARD  = 'white'  # use on dark boxes/panels
COLOR_ON_PLAIN = 'white'  # use directly on the  background
...
# ---------- Balloon growth ----------
START_RADIUS = 20          # small start (diameter 20 px)
MIN_RADIUS   = START_RADIUS
GROWTH_PCT   = 0.100       # per pump; at least +1 px enforced below
...
def csv_append(path, header, row):
    ...
    with open(path, 'a', newline='', encoding='utf-8') as f:
        ...
        if write_header:
            w.writerow(header)
        w.writerow(row)
def log_event(participant, trial, event_name, event_time):
    csv_append(
        EVENTS_CSV,
        header=['participant_id','trial','event','time'],
        row=[participant, trial, event_name, f"{event_time:.4f}"]
    )
...
# Collect/Bank sound
try:
    collect_sound = sound.Sound(COLLECT_SOUND_FILE)
except Exception:
    collect_sound = None

# Pop sound
try:
    pop_sound = sound.Sound(POP_SOUND_FILE)
except Exception:
    pop_sound = None
...
def play_pop_once(pos_pix, radius):
    ...
    pop1 = safe_image(POP1_PNG, size=frame_size, pos=pos_pix)
    pop2 = safe_image(POP2_PNG, size=frame_size, pos=pos_pix)
    if pop_sound:
        try: pop_sound.play()
...
def show_intro_single_page():
    ...
    title = visual.TextStim(
        ...
        units='pix', pos=(0, 160),
        color=(1.0000, 1.0000, 1.0000), font=FONT_MAIN, height=40,
    ...
    body = visual.TextStim(
        ...
        units='pix', pos=(0, -10),
        color=(1.0000, 1.0000, 1.0000), font=FONT_MAIN, height=26,
        alignText='center', wrapWidth=1000
    )
    footer = visual.TextStim(
        win, text="Press SPACE to start practice",
        units='pix', pos=(0, -280),
        color=(1.0000, 1.0000, 1.0000), font=FONT_MAIN, height=24,
        alignText='center'
    )
...
def draw_hud_by_radius(radius_px, color_name, tempBank, permBank, count_text):
    ...
    # Right cards and labels/values (white on dark)
...
# ---------- Practice ----------
def run_practice(trials, subject_id):
    permBank = 0.0
    for idx, trial in enumerate(trials, start=1):
        ...
        while continuePumping and not pop:
            ...
            if not key:
                ...
                pump_time = experiment_clock.getTime()
                log_event(subject_id, idx, "pump", pump_time)
...
def bart():
    ...
    # Practice
    _ = run_practice(create_practice_trials(), subject_id)
    ...
    # EEG synchronization marker
    log_event(subject_id, 0, "task_start", experiment_clock.getTime())
    ...
    for idx, trial in enumerate(trials, start=1):
        ...
        while continuePumping and not pop:
            ...
            if not key:
                ...
                cashout_time = experiment_clock.getTime()
                log_event(subject_id, idx, "cashout", cashout_time)
                ...
                if nPumps >= E:
                    explosion_time = experiment_clock.getTime()
                    log_event(subject_id, idx, "explosion", explosion_time)
    ...
    # EEG synchronization marker
    log_event(subject_id, 0, "task_end", experiment_clock.getTime())
